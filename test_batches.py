...
def test(config_file):
    ...
    for i in range(image_num):
        ...
        print(volume_report)
        #description can only store 80 characters
        simplied_volume_report = f"Total tumor:{tumor_volume['total tumor volume']} {tumor_volume['unit']} " \
                       f"(enhancing:{tumor_volume['enhancing portion']} {tumor_volume['unit']}; " \
                       f"non enhancing:{tumor_volume['non enhancing portion']} {tumor_volume['unit']})" \
                       f"Edema:{tumor_volume['total vasogenic edema volume']} {tumor_volume['unit']}. \n"
        add_tumor_volume_to_nifty(subfolder+"/{}_seg_whole.nii.gz".format(temp_name.split('/')[-1]), simplied_volume_report)
        with open(subfolder + "/{}_volume_report.txt".format(temp_name.split('/')[-1]), 'w') as volume_file:
            volume_file.write(volume_report)

        #save tumor seperately, edema, enhancing & non_enhancing
        final_label_edema = final_label.copy()
        final_label_edema[final_label_edema != 2] = 0
        save_array_as_nifty_volume(final_label_edema,
                                   subfolder + "/{}_seg_edema.nii.gz".format(temp_name.split('/')[-1]), img_names[0])
        #add volume of tumor into nifty file
        #todo save it in simpleitk, not by nibabel
        volume_report_edema = f"Total vasogenic edema volume: {tumor_volume['total vasogenic edema volume']} {tumor_volume['unit']}."
        add_tumor_volume_to_nifty(subfolder + "/{}_seg_edema.nii.gz".format(temp_name.split('/')[-1]), volume_report_edema)

        final_label_enhanced = final_label.copy()
        final_label_enhanced[final_label_enhanced != 4] = 0
        save_array_as_nifty_volume(final_label_enhanced,
                                   subfolder + "/{}_seg_enhanced.nii.gz".format(temp_name.split('/')[-1]), img_names[0])
        volume_report_enhanced = f"Tumor volume(enhancing portion): {tumor_volume['enhancing portion']} {tumor_volume['unit']}"
        add_tumor_volume_to_nifty(subfolder + "/{}_seg_enhanced.nii.gz".format(temp_name.split('/')[-1]), volume_report_enhanced)

        final_label_non_enhanced = final_label.copy()
        final_label_non_enhanced[final_label_non_enhanced != 1] = 0
        save_array_as_nifty_volume(final_label_non_enhanced,
                                   subfolder + "/{}_seg_non_enhanced.nii.gz".format(temp_name.split('/')[-1]), img_names[0])
        volume_report_non_enhanced = f"Tumor volume(non enhancing portion): {tumor_volume['non enhancing portion']} {tumor_volume['unit']}"
        add_tumor_volume_to_nifty(subfolder + "/{}_seg_non_enhanced.nii.gz".format(temp_name.split('/')[-1]), volume_report_non_enhanced)
...
if __name__ == '__main__':
    if(len(sys.argv) != 3):
        ...
        print('python test.py config17/test_all_class.txt /gpfs/scratch/bz957/brats/data/langone/post/post_5cases_t1ce2t1_f2t2')
        exit()
    config_file = str(sys.argv[1])
    data_file = str(sys.argv[2])
    basename_data = os.path.basename(data_file)
    config_folder = config_file.split('/')[0]
    assert (os.path.isfile(config_file))

    # for folder in os.listdir(data_file):
    #     if not folder.startswith('.'):
    #         file = open('/gpfs/scratch/bz957/brats/config17/test_names_temp.txt', 'w')
    #         print(f'data/langone/post/{basename}/{folder}\n')
    #         file.write(f'data/langone/post/{basename}/{folder}\n')
    #         file.close()
    #         test(config_file)

    file = open(f'/gpfs/scratch/bz957/brats/{config_folder}/test_names_temp.txt', 'w')
    for case in os.listdir(data_file):
        if not case.startswith('.'):
            print(f'write the path data/langone/post/{basename_data}/{case} into {config_folder}/test_names_temp.txt')
            file.write(f'data/langone/post/{basename_data}/{case}\n')
    file.close()
    test(config_file)
